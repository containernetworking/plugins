...
// Note: Although two transactions are taken place, only the 2nd one where the rules
...
func (sc *SpoofChecker) Teardown() error {
	...
	if ifaceMatchRuleErr == nil {
		...
		rule := lookupRule(currentConfig, expectedRuleToFind)
		if rule != nil {
			c := nft.NewConfig()
			c.DeleteRule(rule)
...
func lookupRule(config *nft.Config, ruleToFind *schema.Rule) *schema.Rule {
	for _, nftable := range config.Nftables {
		if nftable.Rule != nil {
			match := nftable.Rule.Table == ruleToFind.Table &&
				nftable.Rule.Chain == ruleToFind.Chain &&
				nftable.Rule.Comment == ruleToFind.Comment
			if match {
				return nftable.Rule
			}
		}

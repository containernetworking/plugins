...
var _ = Describe("dummy Operations", func() {
	...
	BeforeEach(func() {
		...
		// Monkeypatch the subprocess call to use the local method invocation
		monkey.Patch(createSelinuxTap, createLink)
	})
	...
	for _, ver := range testutils.AllSpecVersions {
		...
		It(fmt.Sprintf("[%s] add, check and remove a tap device run correctly", ver), func() {
			if selinux.EnforceMode() != -1 {
				// Ensure each tests starts with a clean selinux exec label
				lock.Lock()
				defer lock.Unlock()
				err := selinux.SetExecLabel("")
				Expect(err).NotTo(HaveOccurred())
			}

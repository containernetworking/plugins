...
var _ = Describe("macvlan Operations", func() {
	...
	It("creates an macvlan link in a non-default namespace", func() {
		...
		err = originalNS.Do(func(ns.NetNS) error {
			...
			_, err = createMacvlan(conf, "foobar0", targetNs, false)
			Expect(err).NotTo(HaveOccurred())
			return nil
		})
		Expect(err).NotTo(HaveOccurred())
		...
		err = targetNs.Do(func(ns.NetNS) error {
			...
			Expect(link.Type()).To(Equal((&netlink.Macvlan{}).Type()))
			return nil
		})
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("creates an macvtap link in a non-default namespace", func() {
		...
		err = originalNS.Do(func(ns.NetNS) error {
			...
			_, err = createMacvlan(conf, "foobar0", targetNs, true)
			Expect(err).NotTo(HaveOccurred())
			return nil
		})
		Expect(err).NotTo(HaveOccurred())
		...
		err = targetNs.Do(func(ns.NetNS) error {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(link.Attrs().Name).To(Equal("foobar0"))
			Expect(link.Type()).To(Equal((&netlink.Macvtap{}).Type()))
			return nil
		})
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("configures and deconfigures a macvtap link from args with ADD/DEL", func() {
		const IFNAME = "macvt0"
    ...
    "ipam": {
		...
		args := &skel.CmdArgs{
			...
			IfName:      IFNAME,
			StdinData:   []byte(conf),
			Args:        "IgnoreUnknown=true;ConfigureNetworkForVM=true",
		}
	...
	It("deconfigures an unconfigured macvtapn link with DEL", func() {
		const IFNAME = "macvt0"
